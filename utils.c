...
void vminfo()
{
    ...
    while (block->size_status != VM_ENDMARK) {
        ...
        if (busy) {
            ...
            if (blocksz > largest_free_size) {
                largest_free_size = blocksz;
                largest_free_blockid = blockid;
            }
        }
    ...
    if (largest_free_blockid >= 0) {
        printf("The largest free block is #%d with size %zu.\n",
               largest_free_blockid, largest_free_size);
    } else {
